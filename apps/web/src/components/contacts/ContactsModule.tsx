...
export function ContactsModule() {
  ...
  const sel = rows.find((c) => c.id === selId) ?? rows[0];
  ...
  if (isMobile) {
    return (
      <div style={{ padding: 8 }}>
        {rows.map((ct) => (
          <div key={ct.id} className="hoverable" style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 10px", borderRadius: 12, cursor: "pointer" }}>
  ...
  const cats = [
    { name: "All Contacts", icon: "👥", count: rows.length, active: true },
    { name: "Favorites", icon: "☆", count: rows.filter((c) => c.favorite).length },
    { name: "Online", icon: "🟢", count: rows.filter((c) => c.presence === "ONLINE").length },
  ];
    ...
    <>
      <div style={{ width: 238, flexShrink: 0, display: "flex", flexDirection: "column", borderRight: "1px solid var(--border)", background: "var(--bg)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 18px 12px" }}>
          ...
          <span style={{ color: "var(--text2)", cursor: "pointer", display: "flex" }}>
            <AddPersonIcon />
          </span>
        </div>
        <div style={{ padding: "0 10px", display: "flex", flexDirection: "column", gap: 2 }}>
          {cats.map((cc) => (
            <div
              key={cc.name}
              className={cc.active ? undefined : "hoverable"}
              style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 11px", borderRadius: 11, cursor: "pointer", fontWeight: 600, fontSize: 13.5, background: cc.active ? "var(--sel)" : "transparent", color: cc.active ? "var(--p600)" : "var(--text)" }}
            >
              <span style={{ opacity: 0.8 }}>{cc.icon}</span>
              <span style={{ flex: 1 }}>{cc.name}</span>
              <span style={{ fontSize: 11.5, fontWeight: 700, borderRadius: 99, padding: "2px 8px", background: cc.active ? "var(--p600)" : "var(--muted)", color: cc.active ? "#fff" : "var(--text2)" }}>{cc.count}</span>
            </div>
          ))}
        </div>
        <div style={{ margin: "10px 18px 0", borderTop: "1px solid var(--border)" }} />
        <div style={{ padding: 10, display: "flex", flexDirection: "column", gap: 2 }}>
          <div className="hoverable" style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 11px", borderRadius: 11, cursor: "pointer", fontWeight: 600, fontSize: 13.5 }}>
          ...
          <div className="hoverable" style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 11px", borderRadius: 11, cursor: "pointer", fontWeight: 600, fontSize: 13.5 }}>
            ...
            <span style={{ fontSize: 11.5, fontWeight: 700, color: "var(--text2)" }}>{rows.filter((c) => c.blocked).length}</span>
          </div>
        </div>
      </div>
      <main style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", background: "var(--bg)", borderRight: "1px solid var(--border)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 18px" }}>
          ...
          <div style={{ width: 38, height: 38, borderRadius: 11, border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text2)", cursor: "pointer" }}>
            <FilterIcon />
          </div>
          <div style={{ marginLeft: "auto", fontSize: 12.5, color: "var(--text2)" }}>
            Sort by: <b style={{ color: "var(--text)" }}>A–Z</b> ▾
          </div>
        </div>
            ...
            return (
              ...
              >
                ...
                <div style={{ display: "flex", gap: 4, justifyContent: "flex-end", color: "var(--text2)" }}>
                  ...
                  <div className="hoverable-sel" style={{ width: 29, height: 29, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "var(--p600)" }}>
                    <PhoneIcon size={13} strokeWidth={1.7} />
                  </div>
                  <div className="hoverable" style={{ width: 29, height: 29, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                    <DotsVIcon size={14} />
                  </div>
                </div>
              </div>
      ...
      {sel && <ContactDetail ct={sel} onMessage={() => openDm.mutate(sel.id)} onFavorite={() => api(`/contacts/${sel.id}/favorite`, { method: "POST" }).then(() => qc.invalidateQueries({ queryKey: ["contacts"] }))} />}
    </>
  );
}
...
function ContactDetail({ ct, onMessage, onFavorite }: { ct: ContactDto; onMessage: () => void; onFavorite: () => void }) {
  const router = useRouter();
  return (
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, width: "100%", marginTop: 16 }}>
          {[
            ...
            { label: "Call", icon: <PhoneIcon size={15} strokeWidth={1.7} /> },
            { label: "Video", icon: <VideoIcon size={16} />, onClick: () => router.push("/app/calls") },
            { label: "More", icon: <DotsHIcon size={16} /> },
          ].map((a) => (
